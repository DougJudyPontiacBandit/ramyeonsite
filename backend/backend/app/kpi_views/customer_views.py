...
class CustomerQRView(APIView):
    """View for customer QR code management"""
    def __init__(self):
        self.customer_service = CustomerService()
    ...
    def post(self, request, customer_id):
        ...
        try:
            updated_customer = self.customer_service.regenerate_qr_code(
                customer_id,
                request.current_user
            )
            if updated_customer:
            ...
            logger.error(f"Error regenerating QR code for customer {customer_id}: {e}")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )

class CustomerQRScanView(APIView):
    """View for scanning customer QR codes (POS integration)"""
    def __init__(self):
        self.customer_service = CustomerService()

    @require_authentication
    def post(self, request):
        """Scan customer QR code and return customer details"""
        try:
            ...
            customer = self.customer_service.get_customer_by_qr_code(qr_code)
            if customer:
                return Response({
                    "customer_id": customer.get('customer_id', ''),
                    "full_name": customer.get('full_name', ''),
            ...
            return Response(
                {"error": "Invalid QR code or customer not found"},
                status=status.HTTP_404_NOT_FOUND
            )
        except Exception as e:
            logger.error(f"Error scanning customer QR code: {e}")
            return Response(
                {"error": str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
