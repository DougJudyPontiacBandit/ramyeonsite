...
class POSTransactionView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Check for customer QR code
            customer_qr = request.data.get('customer_qr')
            customer_info = None
            if customer_qr:
                from ..services.customer_service import CustomerService
                customer_service = CustomerService()
                customer = customer_service.get_customer_by_qr_code(customer_qr.strip())
                if customer:
                    customer_info = {
            ...
            if result['success']:
                # Award loyalty points to customer if customer_info available (from QR scan)
                customer_id = None
                if customer_info:
                    customer_id = customer_info['customer_id']
                else:
                    # Fallback to customer_id from request
                ...
                if customer_id:
                    try:
                        ...
                        points_to_award = int(total_amount)  # 1 point per dollar

                        if points_to_award > 0:
                            customer_service.update_loyalty_points(
                                customer_id,
                                points_to_award,
                                f"POS Purchase - Transaction {result.get('transaction', {}).get('transaction_id', '')}",
                                current_user
                            )
                            result['points_awarded'] = points_to_award
                            result['customer_info'] = customer_info
                    except Exception as points_error:
                        logging.warning(f"Failed to award points to customer {customer_id}: {points_error}")
