...
class Customer:
    def __init__(self, **kwargs):
        ...
        self.qr_code = kwargs.get('qr_code', self.generate_qr_code())
    ...
    def generate_qr_code(self):
        """Generate a unique QR code for the customer"""
        import uuid
        return f"CUST-{str(self._id)[:8]}-{str(uuid.uuid4())[:8]}".upper()
...
class Cart:
    def __init__(self, **kwargs):
        self._id = kwargs.get('_id', ObjectId())
        self.customer_id = kwargs.get('customer_id', None)  # Can be None for guest carts
        self.items = kwargs.get('items', [])  # List of cart items
        self.total_amount = kwargs.get('total_amount', 0.0)
        self.status = kwargs.get('status', 'active')  # active, completed, abandoned
        self.date_created = kwargs.get('date_created', datetime.utcnow())
        self.last_updated = kwargs.get('last_updated', datetime.utcnow())
